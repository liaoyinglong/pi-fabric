export const WORKFLOW_COMPAT_GUEST_PREAMBLE =
  ...
  "if(typeof task!==\"string\"||!task.trim())throw new TypeError(\"workflow.agent object form requires a non-empty task string\");" +
  ...
  "if(hasFunctions&&hasNonFunctions)throw new TypeError(\"workflow.parallel cannot mix functions with already-started values/promises\");" +
  "if(hasNonFunctions){if(arg2!==undefined)throw new TypeError(\"workflow.parallel cannot enforce a concurrency limit after promises have started; pass functions or use Promise.all\");" +
  ...
  "(globalThis as any).workflow=Object.freeze({...__fabricBaseWorkflow,agent:__fabricCompatAgent,parallel:__fabricCompatParallel});";
